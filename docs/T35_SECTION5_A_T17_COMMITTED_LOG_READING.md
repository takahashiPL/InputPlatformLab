# T35 §5 A — T17 ログと committed （読み分け結論）
...
## 今回の判断（T35 §5 A/D 判断同期）

- **A を本体として先に読む**。
- **D は補助**として、`axis=mode` / `axis=committed` の読み違え防止に使う。
- **B/C は今回まだ触らない**。
- 今回ここで固定するのは、**T17 logs と committed を同じ軸として読まない**ことまで。


## 今回の読解結論（A 本体）

- **T17 logs は `axis=mode` 側**として読む。
- **committed は `axis=committed` 側**として読む。
- **両者は別軸**なので、数値差だけで不整合とは読まない。
- 今回ここで固定するのは **読み分け**までで、**数値を揃える話や実装変更には進まない**。

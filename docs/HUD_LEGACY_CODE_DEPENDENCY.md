...
**フェーズ区切り**: レガシー縦積み **TU 分離フェーズ**の close サマリは **`docs/HUD_LEGACY_STACKED_PHASE_CLOSE.md`**（詳細は本書 **§8**）。
...
§8.3 B の各項目について **移設前に固めるべきデータ契約／呼び出し契約**は **§8.4**。**実装で進めるか当面残すか（Go/No-Go）**は **§8.5**。
...
### 8.5 §8.3 B の Go / No-Go（実装で進めるか／当面 main に残すか）

**目的**: §8.4 まで揃えた **契約**を踏まえ、§8.3 B の各項目を **次の実装スプリントで動かすか**、**当面 docs＋現コードのままにするか**を判断する材料にする。**本節も確定ロードマップではない**（優先度・リソースで変わり得る）。

**推奨ラベルの意味**

| ラベル | 意味 |
|--------|------|
| **Go** | 前提が揃えば **実装リファクタを計画してよい**（工数・回帰手順を別途確保）。 |
| **No-Go（当面）** | **現状の main／既存 TU 配置を維持**が合理的。**§8.4 の契約遵守**が当面の完了点。 |
| **後回し** | **いずれ検討**だが、**先に別タスク**（例: struct 化、計測、マクロ 0 CI）の方が費用対効果が高い。 |

#### 8.5.1 項目別（§8.3 B）

| 項目 | 移すメリット | 移す前提条件 | 移したときのリスク | 今は main（現配置）に残す合理性 | 現時点の推奨 |
|------|-------------|-------------|-------------------|-------------------------------|-------------|
| **T46**（`s_t46LastSi*`） | 単体テストしやすい **型**にできる。読み手（FormatScroll/ScrollLog）と **明示的なデータ契約**になる。 | **§8.4.3** の struct 化と **更新者／無効化者**の一覧がコードまたはヘッダで固定済み。 | **TU をまたぐと** 更新順・フレーム境界のミスで **[scroll] SI 行がズレる**。 | **T45 と同一関数内**で更新され、**行数・依存が限定的**。 | **後回し**（まず **同一 TU 内 struct 化**があれば **その次**に移設を再評価）。**いきなり別 TU へ物理移動は No-Go（当面）**。 |
| **T45 本体** | `Win32DebugOverlay.cpp` の **責務が視覚的に細る**。Win32 スクロールバー処理の **専用 .cpp** に集約できる。 | **InvokeT45** を **同じ移設単位**で動かす。**fill-monitor／Windowed** の分岐テスト方針。**§8.4.1** の unified ①②③ を変えないことのレビュー。 | **SI・`s_paintScrollY`・本文描画**の不一致（§8.3 C）。回帰コスト **高**。 | **legacy から 1 ホップ**で済み、**既に安定した境界**。通常運用は **ページ式**で本経路の相対重要度は限定的。 | **No-Go（当面）**。**Go にするなら**「専用リファクタ枠＋レガシー縦積み手動回帰」が揃った **後**。 |
| **`FormatScroll` の文言生成のみ** | `Win32DebugOverlay.cpp` の **行数削減**、表示系の **編集が局所化**。 | **§8.4.4**／**§8.4.3** が固定済み。**SI を引数に載せるか static か**の方針が一択。 | 分離により **二重フォーマット**・未初期化 SI 表示。 | **現状サイズでは分割メリットが小さい**（§8.3 B）。 | **後回し**（**肥大化**または **T46 struct 化完了後**に再評価）。 |
| **`ResetProvisionalLayoutCache` の内訳** | T52／暫定付随／T46 無効の **束ね**を legacy 側に寄せ、**main のオーケストレーション**を薄くできる。 | **§8.4.5** の呼び出し元一覧と **順序**の合意。T46 の **置き場**が決まっていること。 | **二重クリア・取りこぼし**で暫定表示と SI の組み合わせが壊れる。 | **API 表面は小さく**、呼び出し元は **grep で把握可能**。 | **No-Go（当面）**。T46 の **移設方針が固まるまで**触らない方が安全。 |

#### 8.5.2 T45 / T46 に対する総合判断（実装を今すぐ進めるか）

| 観点 | 結論 |
|------|------|
| **§8.4 の契約整理** | **当面の完了点として十分**。ここまでで **「動かす前に何を固定するか」**は判断可能。 |
| **T45 の実装移設** | **今すぐ Go にしない（No-Go 推奨）**。メリットは主に **ファイル分割の見た目**であり、**リスク対効果**が釣り合いにくい。進めるなら **別枠の回帰**が前提。 |
| **T46 の実装移設** | **T45 より後**が自然。**まず同一 TU 内での struct 化**（型＋更新点の明示）までを **次の候補**とし、**別 TU への物理移動**は **さらに後回し**。 |
| **契約整理で止めるべきか** | **はい（当面）**。ページ式が正のまま、**大規模な scroll/SI 変更をしない**方針と整合する。 |
...
| 2026-04-06 | **§8.5 追加**: §8.3 B の **Go / No-Go / 後回し**（移すメリット・前提・リスク・当面残す理由）。T45/T46 は **当面契約整理で止め、実装移設は No-Go／後回し**を明示（挙動不変） |
| 2026-04-06 | **`HUD_LEGACY_STACKED_PHASE_CLOSE.md` 追加**: 分離フェーズ **closed** の短いまとめ（本書冒頭にリンク） |

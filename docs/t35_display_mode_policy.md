...
**観測性（T35 §5 第2歩、policy 変更ではない）**: 実装のデバッグログ・コメントでは `axis=mode` / `axis=committed` / `axis=offscreen` / `axis=final` を接頭辞として使い、**T17 のウィンドウ話**・**T14 committed**・**[T34][RT]/[T36][RT]**・**最終 HUD 帯**を読み分ける（数値が一致しない場面は「軸が違う」ことがある）。

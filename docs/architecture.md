...
| **app-specific glue / verification** | `MainApp.cpp` / `MainApp.h`、`T18InventorySnapshotGlue.*`（T18 snapshot 型・Win32InputGlue survey 後の classify / rationale / HUD 短・`[T18]` debug 行）、`.rc` / `resources/`、InputPlatformLab 固有の T18/T19/T20 文言・検証フロー・`_DEBUG` ホットキー |

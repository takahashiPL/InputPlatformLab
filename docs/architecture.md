...
│   ├── input/
│   │   └── core/            # InputCore 束（候補 A/B など HWND 非依存の中立ヘッダ）
│   ├── platform/
│   │   └── win/             # Windows / Win32 専用のヘッダ
│   └── …                    # その他（例: PlayerInputGuideTypes、分類ヘッダなど）
├── src/                     # 実装本体
│   ├── input/
│   │   └── core/            # 候補 A/B の中立 .cpp（VirtualInputNeutral / LogicalInputState）
│   ├── platform/
│   │   └── win/             # Windows 専用の .cpp（レンダラ・GDI オーバーレイ）
│   └── …                    # その他（例: ControllerClassification.cpp）
...
- 中立層（`CommonTypes.h`、`GamepadTypes.h`、`VirtualInputNeutral.h`、`VirtualInputMenuSample.h`、`LogicalInputState.h`、`ControllerClassification.h` など）は **Win32 HWND を含まない**設計に寄せています。**候補 A/B の主要ヘッダ**（分類ヘッダを除く中心部）は現状 **`include/input/core/`** に配置されています。
- `InputCore.h` は、これら中立ヘッダの **単一入口**としてのコメント付きインクルード集約です（実装は対応する `.cpp` をリンク）。

### `src/` の役割

- **実装の詳細**（関数本体・静的ヘルパ・状態）です。
- 中立ロジックのうち **候補 A/B**（`VirtualInputNeutral.cpp`、`LogicalInputState.cpp`）は **`src/input/core/`** に配置されています（**`WndProc` / `WM_*` / `InvalidateRect` 非接触**のまま）。`ControllerClassification.cpp` などは **`src/`** 直下。
...
- `MainApp.vcxproj` の `AdditionalIncludeDirectories` に `include`、`include\input\core`、`include\platform\win` が入っており、`#include "framework.h"` のように **短い名前**でインクルードできます。
...
| **input/core 相当** | `VirtualInput*` / `LogicalInput*`、プレイヤースロット、arbiter、列挙・定数（HWND 非依存） | **`include/input/core/`**（`InputCore.h`、`CommonTypes` / `GamepadTypes` / `VirtualInputMenuSample` / `VirtualInputNeutral` / `LogicalInputState` など）、**`src/input/core/`**（`VirtualInputNeutral.cpp`、`LogicalInputState.cpp`）、`include/PlayerInputGuideTypes.h` / `PlayerInputSlots.h`、`EffectiveInputGuideArbiter.*`、`ControllerClassification.*` |
...
| 区分 | 中身の例 |
|------|-----------|
| **reusable candidate（portable foundation）** | `PlayerInputGuideTypes.h`、`PlayerInputSlots.h`。**候補 A**: `include/input/core/`（`InputCore.h`、`CommonTypes`、`GamepadTypes`、`VirtualInputMenuSample`、`VirtualInputNeutral.*`）+ `src/input/core/VirtualInputNeutral.cpp`。**候補 B（unit-1 完了）**: `include/input/core/LogicalInputState.*` + `src/input/core/LogicalInputState.cpp`。ほか `ControllerClassification.*`、`EffectiveInputGuideArbiter.*`。**ホスト**（`WndProc`、`WM_INPUT` / `WM_TIMER` / `WM_PAINT`、`InvalidateRect`、T19/T20 本文・accepted）は **未 pack-out**（`docs/ARCHITECTURE_PACKOUT_REUSE_BOUNDARY_NEXT.md` と同趣旨）。 |
...
| **app-specific glue / verification** | `MainApp.cpp` / `MainApp.h`、`T18InventorySnapshotGlue.*`（snapshot / survey後 classify・rationale・HUD 短・debug 行）、`T18PageBodyFormatGlue.*`（T18 ページ本文の compact 行・2P–4P 1行・本文 `swprintf` 束）、`.rc` / `resources/`、InputPlatformLab 固有の T18/T19/T20 文言・検証フロー・`_DEBUG` ホットキー |
| **generated / local-only** | `.vs/`、`Debug/` / `Release/` / `x64/` / `x86/`、`ipch/`、`*.obj` `*.pdb` `*.exe` など（ルート [`.gitignore`](../.gitignore) 参照）。**コピー対象にしない** |

### 他プロジェクトへ持ち出す最小イメージ

1. **Portable pack**: 上表の reusable candidate に対応する `include/input/core/*.h`（および `include/` 直下のスロット型など）と中立 `.cpp`（`src/input/core/*` ほか）をまとめ、ホスト側でビルドに追加する。  

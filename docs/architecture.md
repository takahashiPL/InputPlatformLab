...
| **platform/win specific** | `include/platform/win/*`、`src/platform/win/*`（レンダラ・オーバーレイ・Win32 HUD ページヘルパ・`Win32InputGlue.*`（Raw 登録・XInput probe・デバイス文字列・GetRawInputDeviceList 集約・先頭接続スロット）など） |
